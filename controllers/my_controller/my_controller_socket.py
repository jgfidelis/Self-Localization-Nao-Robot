...
class Nao (Robot):
    ...
    def testSocket(self):
        outer = np.array(self.cameraTop.getImageArray())
        HOST = 'localhost'
        PORT = 50014
        ...
        y = 0
        y = y + 1;
        # outer = np.array(self.camera.getImageArray())
        # plt.imshow(outer.astype(np.uint8))
        # plt.pause(0.0001)
        if y > 10:
            y = 0
            scaled = 2  # defines by which factor the image is subsampled
            width = self.cameraTop.getWidth()
            height = self.cameraTop.getHeight()

            # read rgb pixel values from the camera
            image = self.cameraTop.getImage()
            ...
            for y in range(0, height / scaled):
                inner = []
                for x in range(0, width / scaled):
                    gray = self.cameraTop.imageGetGray(image, width, x * scaled,
                                                       y * scaled) * 9 / 255  # between 0 and  instead of 0 and 255
                    inner.append(gray)
                outer.append(gray)
            dump = pickle.dumps(outer)
